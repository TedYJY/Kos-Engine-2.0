...
class AcidGrenadeGasAoe : public TemplateSC {
public:
	int     gasDamage = 2;        // Damage per tick
	float   tickInterval = 1.f;      // Damage every 1 second
	float   lifetime = 4.f;      // Gas cloud lives for 4 seconds
	int     scoreValue = 50;       // Score per kill inside gas
	...
	REFLECTABLE(AcidGrenadeGasAoe, gasDamage, tickInterval, lifetime, scoreValue)
};
...
inline void AcidGrenadeGasAoe::Start() {
...
inline void AcidGrenadeGasAoe::Update() {
